...
actions | 是 | 数组 | 添加可交互的组件。支持添加的组件如下，你可以跳转至对应的组件文档查看参数配置详情。<br>- [按钮（button）](https://open.larksuite.com/document/ukTMukTMukTM/uEzNwUjLxcDM14SM3ATN)<br>- [列表选择器（selectMenu）](https://open.larksuite.com/document/ukTMukTMukTM/uIzNwUjLycDM14iM3ATN)<br>- [折叠按钮组（overflow）](https://open.larksuite.com/document/ukTMukTMukTM/uMzNwUjLzcDM14yM3ATN)<br>- [日期选择器（datePicker）](https://open.larksuite.com/document/ukTMukTMukTM/uQzNwUjL0cDM14CN3ATN)
...
![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/0e393b3928e49b1c57cf9907f9c32fd2_TlDvcGXBpo.png?height=126&lazyload=true&maxWidth=400&width=770)
...
text | 是 | Object | 单个文本内容，参数配置详情可参见[文本组件](https://open.larksuite.com/document/ukTMukTMukTM/uUzNwUjL1cDM14SN3ATN)。<br>JSON 示例配置：<br>```JSON<br>{<br>"tag": "div",<br>"text": {<br>"tag": "plain_text",<br>"content": "Content module"<br>}<br>}<br>```<br>**注意事项**：**说明**：在内容模块中，`text` 和 `fields` 字段需至少设置一个。
fields | 否 | Object | 多列文本，参数配置详情可参见[双列文本](https://open.larksuite.com/document/ukTMukTMukTM/uYzNwUjL2cDM14iN3ATN)。<br>JSON 示例配置：<br>```JSON<br>{<br>"tag": "div",<br>"fields": [<br>{<br>"text": {<br>"tag": "lark_md",<br>"content": "**module:**\nContent module（div）"<br>},<br>"is_short": true<br>},<br>{<br>"text": {<br>"tag": "lark_md",<br>"content": "**function:**\nNew function"<br>},<br>"is_short": true<br>}<br>]<br>}<br>```<br>**注意事项**：**说明**：在内容模块中，`text` 和 `fields` 字段需至少设置一个。
extra | 否 | Object | 附加元素，添加后展示在文本右侧。支持附加的元素：<br>- 图片（image）<br>- [按钮（button）](https://open.larksuite.com/document/ukTMukTMukTM/uEzNwUjLxcDM14SM3ATN)<br>- [列表选择器（selectMenu）](https://open.larksuite.com/document/ukTMukTMukTM/uIzNwUjLycDM14iM3ATN)<br>- [折叠按钮组（overflow）](https://open.larksuite.com/document/ukTMukTMukTM/uMzNwUjLzcDM14yM3ATN)<br>- [日期选择器（datePicker）](https://open.larksuite.com/document/ukTMukTMukTM/uQzNwUjL0cDM14CN3ATN)<br>JSON 示例配置：<br>```JSON<br>// 如需使用该 JSON 示例，则注意需要手动清除 // 开头的注释<br>{<br>"tag": "div",<br>"extra": {<br>"tag": "img", // 附加一张图片<br>"img_key": "f32****5cf7",<br>"alt": {<br>"tag":<br>"plain_text",<br>"content": "alt_content"<br>}<br>}<br>}<br>```
...
![图片](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/ark/b27831148bee12724ef78f10c409e77f.png?height=297&lazyload=true&maxWidth=400&width=606)
...
![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/c3ab380b17ee4d416aa4a8c3d206ac4e_IPujGHGW5I.png?height=270&lazyload=true&maxWidth=400&width=594)
...
![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/c5fcef1cccc37fcd1c357dc0c7a77953_1HTy3hBxa9.png?height=218&lazyload=true&maxWidth=600&width=1328)

假如在 PC 端点击该示例卡片，会自动跳转到 pc_url 对应的地址。

![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/9219e7ec55737d0392c88b3dc1c1d5ef_JJ0K6jETNh.png?height=1384&lazyload=true&maxWidth=600&width=2842)
...
![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/e2904de952f30f2d892d9880749f0a77_lq1MYAJKMa.png?height=304&lazyload=true&maxWidth=400&width=836)
...
参数 | 是否必须 | 类型 | 说明
---|---|---|---
text | 否 | Struct | 选项显示的内容。<br>- 基于文本组件的 plain_text 模式设置文本内容，详情参见[文本组件](https://open.larksuite.com/document/ukTMukTMukTM/uUzNwUjL1cDM14SN3ATN)。<br>- 当列表选择器（selectMenu）的模式为人员选择器（select_person）时，无需配置 `text` 字段，除此之外，其他场景中 `text` 字段必填。
...
![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/6e86652804a30c1b289fbe8eef1cd2ba_zNBRrsczCb.png?height=200&lazyload=true&maxWidth=600&width=1312)
