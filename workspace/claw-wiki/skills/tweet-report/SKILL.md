...
先用 `exec` 启动脚本：
```
exec(command="python3 /root/.openclaw/workspace/claw-wiki/skills/tweet-report/scripts/gen_tweet_report.py", timeout=1500)
```
...
用 `process` 轮询，直到脚本退出：
```
process(action=poll, sessionId=<第一步返回的sessionId>, timeout=1200000)
```
...
脚本成功后，用 `message` 发送文件（`chat_id` 从消息上下文 `conversation_label` 字段获取，格式 `oc_xxx`）：
```
...
关键字存储在：`/root/.openclaw/workspace/claw-wiki/skills/tweet-report/tweet_keywords.json`

**查看关键字**：
```bash
cat /root/.openclaw/workspace/claw-wiki/skills/tweet-report/tweet_keywords.json
```

**添加关键字**：
```bash
KW_FILE="/root/.openclaw/workspace/claw-wiki/skills/tweet-report/tweet_keywords.json"
python3 -c "
