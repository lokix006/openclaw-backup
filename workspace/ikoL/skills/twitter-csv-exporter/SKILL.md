...
```
exec(command="set -a && source /root/.openclaw/workspace/ikoL/skills/twitter-csv-exporter/.env && set +a && mkdir -p /tmp/openclaw && /root/.bun/bin/bun run /root/.openclaw/workspace/ikoL/skills/twitter-csv-exporter/scripts/fetch-tweets.ts --query \"<搜索词>\" --api-key \"$APIDANCE_API_KEY\" --output \"/tmp/openclaw/<搜索词>.csv\" --limit 50")
```
